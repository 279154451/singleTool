...
public class GridRvDividerDecoration extends RecyclerView.ItemDecoration {
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
        ...
        if(orientation == OrientationHelper.VERTICAL && (position + 1) % spanCount == 0) {//最后一列，则不需要绘制右边线
            outRect.set(0, 0, 0, mDivider.getIntrinsicHeight());
            return;
        }

        if(orientation == OrientationHelper.HORIZONTAL && (position + 1) % spanCount == 0) {//最后一行，不需要绘制底部
    ...
    private void drawVerticalDivider(Canvas canvas, RecyclerView parent, int position) {
        ...
        mDivider.setBounds(left, top, right, bottom);//确定左上角和右下角位置
